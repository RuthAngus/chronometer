...
def action(ra_deg, dec_deg, d_kpc, pm_ra_masyr, pm_dec_masyr, v_los_kms):
    """
    Use Wilma Trick's code to calculate vertical action from
    params:
    ------
    ra_deg: (float)
        RA in degrees.
    ...
    d_kpc: (float)
        Distance in kpc,
    pm_ra_masyr: (float)
        RA proper motion in milliarcseconds/year
    pm_dec_masyr: (float)
        declination proper motion in milliarcseconds/year
    v_los_kms: (float)
...
def calc_actions(R_kpc, phi_rad, z_kpc, vR_kms, vT_kms, vz_kms):
    """
    Calculate the actions. Run from within the action function above.
    """
