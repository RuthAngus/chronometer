...
This version uses only the iso and gyro information.
"""
...
def lnprob(params, *args):
    ...
    return gyro_lnlike + iso_lnlike + lnprior(params, *args)
...
def MH(par, lnprob, nsteps, niter, t, *args):
    ...
    for k in range(niter):  # loop over iterations
        if (k + 1) % 100 == 0:
            print("iteration", k + 1, "of", niter, "iso and gyro only")
...
if __name__ == "__main__":
    ...
    if str(sys.argv[1]) == "combine":
        fn_list = ["0_iso_gyro", "1_iso_gyro", "2_iso_gyro", "3_iso_gyro",
                   "4_iso_gyro", "5_iso_gyro"]
        fn = "combined_samples_iso_gyro"
